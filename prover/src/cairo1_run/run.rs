use super::runner_error::CairoRunError;
use cairo_lang_casm::{
...
pub fn cairo_run_program(
...
) -> Result<(CairoRunner, Vec<MaybeRelocatable>, Option<String>), CairoRunError> {
    ...
    {
        return Err(CairoRunError::IlegalInputValue);
    };
    if cairo_run_config.copy_to_output()
        && !check_only_array_felt_return_type(return_type_id, &sierra_program_registry)
    {
        return Err(CairoRunError::IlegalReturnValue);
    };
...
fn create_entry_code(
...
) -> Result<(CasmContext, Vec<BuiltinName>), CairoRunError> {
    ...
    for ty in &signature.param_types {
        let info = get_info(sierra_program_registry, ty)
            .ok_or_else(|| CairoRunError::NoInfoForType(ty.clone()))?;
    ...
    if expected_arguments_size != actual_args_size {
        return Err(CairoRunError::ArgumentsSizeMismatch {
...
fn fetch_return_values(
...
) -> Result<Vec<MaybeRelocatable>, CairoRunError> {
    if fetch_from_output {
        ...
        if panic_flag {
            return Err(CairoRunError::RunPanic(
                return_values
                    .iter()
    ...
    if let Some(inner_type_size) = result_inner_type_size {
        ...
        if return_values.first() != Some(&MaybeRelocatable::from(0)) {
            // In case of failure, extract the CairoRunError from the return values (aka last two values)
            let panic_data_end = return_values
                .last()
                .ok_or(CairoRunError::FailedToExtractReturnValues)?
                .get_relocatable()
                .ok_or(CairoRunError::FailedToExtractReturnValues)?;
            ...
            if return_values.len() < inner_type_size as usize {
                return Err(CairoRunError::FailedToExtractReturnValues);
            }
            return_values =
